...
class D32X2 {
  ...
  update(data, capabilities, adaptabilities) {
    let vis = this
    vis.data = data || vis.data
    vis.t = () => d3.transition().duration(1000)


    const adaptReducer = (items, d) => ( 
      items.reduce(
        (acc, curr) => Number(d[curr.name]) + acc,
        0
      ) / items.length || 2.5
    ).toFixed(1)
    
    const scoreReducer = (items, d) => ( 
      items.reduce(
        (acc, curr) => Number(d[curr.name]) + acc,
        0
      ) / items.length * 5 || 2.5
    ...
    const adaps = adaptabilities.filter(d => d.scale_type === "score")
