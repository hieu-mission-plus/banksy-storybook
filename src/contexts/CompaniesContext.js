...
const multipleChoiceFilter = filter => company => {
  const selectedFilters = filter.values.filter(v => v.selected).map(v => v.name)
  return (
    !selectedFilters.length || selectedFilters.includes(company[filter.key])
  )
}

const radioFilter = filter => company =>
  ...
  switch (action.type) {
    ...
    case "SET_FILTER":
      ...
      const filteredCompanies = filters.reduce(
        (acc, curr) => acc.filter(filterFns[curr.key](curr)),
        state.companies
      )
      return { ...state, filters, filteredCompanies }
    case "RESET_FILTERS":
      // filters which shouldn't update
      const nonDisplayFilters = state.filters.filter(f => !f.display)
...
const setCompaniesData = dispatch => (companies, comparison) => {
  const data = companies
    .map(company => ({
      ...company,
      ...comparison.find(comp => comp["Company"] === company["Company"]),
...
const setSelectedCompanies = dispatch => companies => {
...
const setCapabilities = dispatch => capabilities => {
  dispatch({ type: "SET_CAPABILITIES", payload: capabilities })
}

const setAdaptabilities = dispatch => adaptabilities => {
